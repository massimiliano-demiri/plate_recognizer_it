...
# ─── Configurazione OCR ─────────────────────────────────────────────────────
UPSCALE_FACTOR = 2  # Moltiplicatore crop
MIN_CONFIDENCE = 40.0  # soglia minima
...
async def load_easyocr_model():
    ...
    logger.info(f"[startup] EasyOCR caricato in {time.perf_counter()-t0:.1f}s")
...
# ─── Trova regioni blu (strisce di targa) ─────────────────────────────────


def detect_blue_strips(img: np.ndarray) -> np.ndarray:
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    # range blu europeo della targa
    lower = np.array([90, 50, 50])
    upper = np.array([130, 255, 255])
    mask = cv2.inRange(hsv, lower, upper)
    # chiudi regioni sottili
    kern = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 15))
    return cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kern)

# ─── Rilevamento regione targa avanzato ────────────────────────────────────
...
def find_plate_region(
    ...
    # Strobo blue strips per guidare riquadro
    blue_mask = detect_blue_strips(img)
    # combina con contorni gray-based
    ...
    # intersezione blu
    fused = cv2.bitwise_and(closed, blue_mask)
    # se vuoto, fallback alla sola closed
    mask = fused if cv2.countNonZero(fused) > min_area else closed
...
# ─── OCR multipasso con upscaling ─────────────────────────────────────────


def ocr_plate(crop: np.ndarray) -> Tuple[Optional[str], float]:
    h, w = crop.shape[:2]
    scaled = cv2.resize(
        crop, (w*UPSCALE_FACTOR, h*UPSCALE_FACTOR), interpolation=cv2.INTER_CUBIC)
    ...
    if not cands:
        return None, 0.0
    plate, conf = max(cands, key=lambda x: x[1])
...
async def recognize_plate(file: UploadFile = File(..., description="JPEG/PNG image")):
    logger.info("[recognize] ricezione %s", file.filename)
    ...
    rect = find_plate_region(vis, gray)
    if not rect:
        logger.info("[recognize] no plate region")
